...
def train_fn(fold):
    ...
    # Data loaders
    train_dataset = Camelyon16(folds.loc[trn_idx].reset_index(drop=True), transform=transformTrain)
    valid_dataset = Camelyon16(folds.loc[val_idx].reset_index(drop=True), transform=transformValid)
    ...
    if CFG.onecyclepolicy == True:
        scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=CFG.lr, div_factor=100, pct_start=0.0,
    ...
    with mlflow.start_run() as run:
        ...
        for epoch in range(CFG.epochs):
            ...
            try:
                ...
                auc_score = roc_auc_score(valid_labels, preds)  # If all labels belong to one class then continue to the next epoch
            ...
            if auc_score > best_auc:
                ...
                LOGGER.debug(f'  Epoch {epoch + 1} - Save Best Score: {best_auc:.4f} Model')
